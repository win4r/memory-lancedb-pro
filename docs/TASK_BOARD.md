...
- [DONE] Freeze the compatibility Markdown target as a per-agent workspace parallel subtree (for example `memory/plugins/memory-lancedb-pro/`) instead of mixing plugin output into human-authored daily logs
...
- [TODO] Wire the runtime Markdown compatibility write-path to the frozen subtree contract (`memory/plugins/memory-lancedb-pro/README.md` + `memory/plugins/memory-lancedb-pro/YYYY-MM-DD.md`) instead of ad-hoc mirror roots
