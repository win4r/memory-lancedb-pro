      ...
      return {
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            ...
            const manualSource = createSourceRecord({
              type: "manual",
              agentId,
              excerpt: text.slice(0, 200),
              confidenceHint: safeImportance,
            });
              ...
              metadata: stringifySmartMetadata(
                buildSmartMetadata(
                  ...
                  },
                  {
                    ...
                    l1_overview: `- ${text}`,
                    l2_content: text,
                    provenance: buildInitialProvenance(manualSource),
                    decision: buildInitialDecision({
                      actor: "user",
                      reason: "Manually stored via memory_store tool",
                      sourceIds: [manualSource.source_id],
                    }),
      ...
      return {
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, runtimeCtx) {
          ...
          try {
            ...
            // Build metadata updates: sync l0/l1/l2 when text changes, append decision trail
            ...
            if (existing) {
              ...
              // If text changed, sync l0/l1/l2 to avoid claim-vs-text inconsistency
              if (text) {
                metaPatch.l0_abstract = text;
                metaPatch.l1_overview = `- ${text}`;
                metaPatch.l2_content = text;
                // Update claim value_summary if claim exists
                if (existingMeta.claim) {
                  metaPatch.claim = { ...existingMeta.claim, value_summary: text };
                }
              }

              // Append decision trail entry
              const updateSource = createSourceRecord({
                type: "feedback",
                agentId,
                excerpt: text ? text.slice(0, 200) : undefined,
              });

              const fieldsChanged = Object.keys(updates).filter(k => k !== "vector").join(", ");
              metaPatch.provenance = {
                sources: [updateSource],
                evidence_count: 1,
                first_observed_at: updateSource.timestamp,
                last_observed_at: updateSource.timestamp,
              };
              metaPatch.decision = appendDecisionEntry(existingMeta.decision, {
                action: "feedback_applied",
                actor: "user",
                timestamp: updateSource.timestamp,
                reason: `Manual update: ${fieldsChanged}`,
                source_ids: [updateSource.source_id],
              });

              updates.metadata = stringifySmartMetadata(
                buildSmartMetadata(existing, metaPatch),
              );
            }
