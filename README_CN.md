...
| `llm.auth` | string | `api-key` | `api-key` 使用 `llm.apiKey` / `embedding.apiKey`；`oauth` 使用插件 OAuth token 文件（路径记录在 `llm.oauthPath`） |
...
| `llm.oauthPath` | string |  | `llm.auth` 为 `oauth` 时使用的 OAuth token 文件 |
...
- OAuth token 文件由 `auth login` 写入，路径记录在 `llm.oauthPath`。
...
4. 回调成功后，命令会保存 OAuth token 文件（路径记录在 `llm.oauthPath`）、为 logout 快照原来的 `api-key` 模式 `llm` 配置，并把插件 `llm` 配置切换为 OAuth 字段（`auth`、`oauthProvider`、`model`、`oauthPath`）
